...
export default class Chat extends Component {
  ...
  sessionHandler = session => {
    this.client.connect(session).then(session => {
      ...
      this.addLogMessage(`User '${topic.self.userId}' joined '${topic.topic.room}' room.`);
  ...
  submitConnect = event => {
    ...
    this.client = new nakamajs.Client(this.state.serverkey, this.state.host, this.state.port);
    ...
    var message = nakamajs.AuthenticateRequest.device(this.device);
    this.client.login(message).then(this.sessionHandler).catch(error => {
      // USER_NOT_FOUND
      if (error.code === 5) {
        this.client.register(message).then(this.sessionHandler).catch(errorHandler);
      } else {
        errorHandler(error);
      }
    });
  ...
  submitMessage = event => {
    ...
    this.client.send(message).then(ack => {
      ...
      <Container fluid>
        ...
        <Form>
          <Form.Group widths='equal'>
            <Form.Field control={Input} label='Host' value={this.state.host} onChange={this.changeField('host')} />
            <Form.Field control={Input} label='Port' value={this.state.port} onChange={this.changeField('port')} />
            <Form.Field control={Input} label='Server Key' value={this.state.serverkey} onChange={this.changeField('serverkey')} />
          </Form.Group>
          <Form.Group inline>
            <Form.Field control={Checkbox} label='Use SSL' checked={this.state.ssl} onChange={this.changeCheckbox('ssl')} />
            <Form.Field control={Button} color='blue' onClick={this.submitConnect}>{
              (this.state.connected) ? 'Disconnect' : 'Connect'
            }</Form.Field>
          </Form.Group>
        </Form>

        <Header size='medium' as='h2' dividing>Messages</Header>
            ...
            messages.map(message => (
              <Comment key={message.messageId}>
                <Comment.Content>
                  ...
                  <Comment.Metadata>
                    <span>{new Date(message.createdAt * 1).toTimeString()}</span>
                  </Comment.Metadata>
                  <Comment.Text>
        ...
        <Form reply>
          <Form.Group inline>
            <Form.Field control={Input} placeholder='Message' width='8'
              value={this.state.message}
              disabled={!this.state.connected}
              onChange={this.changeField('message')}
            />
            <Form.Field control={() => (
              <Button content='Send' labelPosition='left' icon='edit' color='blue'
                disabled={!this.state.connected || this.state.message.length < 1}
        ...
        <Header size='medium' as='h2'>Logs</Header>
        ...
        <Segment attached='bottom' style={{ padding: '0', height: '150px', overflowY: 'scroll' }}>
          <Table compact singleLine striped style={{ border: '0' }}>
            <Table.Body>{
              logs.map(log => (
                <Table.Row key={log.ts}>
                  <Table.Cell width='5'>{new Date(log.ts).toTimeString()}</Table.Cell>
